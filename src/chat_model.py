...
class ChatModel:
    ...
    def _ensure_local_model(self, model_name: str) -> None:
        ...
        try:
            ...
            logger.info("Local chat model loaded from local cache: %s", model_name)
        except Exception as local_exc:
            # 检查是否为本地缺失，若是则联网下载
            msg = str(local_exc).lower()
            missing_keywords = [
                "not found in local", "could not find", "couldn't find", "can't find",
                "no such file or directory", "is not the path to a directory containing",
                "does not appear to have a file named", "cannot find the requested files",
                "cannot find requested files"
            ]
            if any(k in msg for k in missing_keywords):
                logger.warning("Chat model not found in local cache, will auto-download: %s", local_exc)
                self._tokenizer = AutoTokenizer.from_pretrained(
                    model_name,
                    trust_remote_code=True,
                ...
                logger.info("Local chat model downloaded and loaded: %s", model_name)
            else:
                raise
